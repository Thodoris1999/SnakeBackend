...
public class Board {
    ...
    public void createBoard() {
        ...
        for (int i = 0; i < snakes.length; i++) {
            ...
            for (int j = 0; j < M * N * 10; j++) {
                headId = r.nextInt(M * N - 2) + 2;
                if (!headIdExists(headId)) break;
            }
            if (headId == 0) throw new IllegalArgumentException("Could not find valid snake head position");
        ...
        for (int i = 0; i < ladders.length; i++) {
            ...
            for (int j = 0; j < M * N * 10; j++) {
                ...
                if (!downstepIdExists(downstepId) && !headIdExists(downstepId))
                    break;
            }
            if (downstepId == 0) throw new IllegalArgumentException("Could not find valid ladder downstep position");

            for (int j = 0; j < snakes.length; j++) {
                if (snakes[j] == null) break;
            ...
            for (int j = 0; j < M * N * 10; j++) {
                ...
                if (snake == null) break;
                if (!(snake.getHeadId() == upstepId)) break;
            }
            if (upstepId == 0) throw new IllegalArgumentException("Could not find valid ladder upstep position");
        ...
        for (int i = 0; i < apples.length; i++) {
            ...
            for (int j = 0; j < M * N * 10; j++) {
                ...
                if (!appleIdInvalid(tile)) break;
            }
            if (tile == 0) throw new IllegalArgumentException("Could not find valid apple position");
            int points;
            do {
